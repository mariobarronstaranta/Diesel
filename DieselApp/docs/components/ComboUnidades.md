...
function ComboUnidades({
  ...
  error,
  cveCiudad,
}: ComboUnidadesProps): JSX.Element;
```
...
Para `SalidasDiesel` se usa **`ComboUnidadesSearchable`** en lugar de este componente.  
`ComboUnidades` queda disponible para uso futuro en otros formularios.

Ver: [ComboUnidadesSearchable.md](./ComboUnidadesSearchable.md)
