...
```typescript
function ComboOperadores({
  register,
  error,
  cveCiudad,
}: ComboOperadoresProps): JSX.Element;
```
...
## Variante con búsqueda predictiva

Para `SalidasDiesel` se usa **`ComboOperadoresSearchable`** en lugar de este componente.  
`ComboOperadores` queda disponible para uso futuro en otros formularios.

Ver: [ComboOperadoresSearchable.md](./ComboOperadoresSearchable.md)
