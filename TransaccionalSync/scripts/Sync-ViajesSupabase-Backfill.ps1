<#
.SYNOPSIS
    Sync-ViajesSupabase-Backfill.ps1 — Sincronización histórica (backfill) de viajes hacia Supabase.

.DESCRIPTION
    Proceso ETL diseñado para reprocesar o enviar carga inicial de rangos de fechas históricos que:
      1. Lee la configuración de conexión desde sync.config.json
      2. Registra el inicio del proceso en SQL Server (Sync_Ejecucion)
      3. Ejecuta sp_SincronizarViajesCierres para obtener los viajes en un rango de fechas.
      4. Si hay datos: envía un INSERT bulk a la API REST de Supabase, ignorando duplicados.
      5. Registra el detalle de los viajes enviados (Sync_Detalle)
      6. Cierra el registro del proceso con el estatus final

    Este script NO está pensado para ejecutarse de forma programada diariamente (para eso está Sync-ViajesSupabase.ps1),
    sino para uso manual puntual por administrador, pasando fecha de inicio y fin.

.PARAMETER FechaInicio
    Fecha inicial del rango a sincronizar (formato YYYY-MM-DD). Obligatorio.

.PARAMETER FechaFin
    Fecha final del rango a sincronizar (formato YYYY-MM-DD). Obligatorio.
...
.EXAMPLE
    # Sincronizar un rango de fechas completo
    .\Sync-ViajesSupabase-Backfill.ps1 -FechaInicio "2026-02-01" -FechaFin "2026-02-28"
...
.NOTES
    ...
    BITÁCORA DE CAMBIOS
    -------------------
    v1.0.0 | 2026-03-11 | [Tu Nombre] | Creación inicial (basado en Sync-ViajesSupabase.ps1)
#>
...
# =============================================================================
# Nombre del archivo: SyncLog_Backfill_FECHAINICIO_FECHAFIN.txt
if (-not (Test-Path $LogDir)) { New-Item -ItemType Directory -Path $LogDir -Force | Out-Null }
$global:LogFile = Join-Path $LogDir ("SyncLog_Backfill_" + ($FechaInicio -replace "-","") + "_" + ($FechaFin -replace "-","") + ".txt")
...
# =============================================================================
Write-Log "========================================================"
Write-Log "Iniciando BACKFILL de sincronización de viajes."
Write-Log "Rango de fechas: Desde $FechaInicio hasta $FechaFin"
Write-Log "========================================================"
...
try {
    ...
    # NOTA: sp_RegistrarInicioSync solo toma un campo de fecha. Le pasaremos $FechaInicio
    # para no alterar el SP original de la tabla de control, pero pondremos nota en el log.
    Write-Log "Registrando inicio del proceso en Sync_Ejecucion (con FechaProceso=$FechaInicio)..."
    $cmdInicio = $sqlConn.CreateCommand()
    $cmdInicio.CommandText = "EXEC dbo.sp_RegistrarInicioSync @FechaProceso, @IDEjecucion OUTPUT"
    $cmdInicio.Parameters.AddWithValue("@FechaProceso", $FechaInicio) | Out-Null
    ...
    Write-Log "Ejecucion registrada con IDEjecucion: $idEjecucion (BACKFILL $FechaInicio al $FechaFin)"

    # ─── 4. Obtener viajes del SP de origen (RANGO) ──────────────────────────
    Write-Log "Ejecutando sp_SincronizarViajesCierres desde $FechaInicio hasta $FechaFin..."
    $cmdViajes = $sqlConn.CreateCommand()
    $cmdViajes.CommandText = "EXEC dbo.sp_SincronizarViajesCierres @FechaInicio, @FechaFin"
    $cmdViajes.CommandTimeout = 300 # Aumentado a 5 mins por si el backfill es masivo
    $cmdViajes.Parameters.AddWithValue("@FechaInicio", $FechaInicio) | Out-Null
    $cmdViajes.Parameters.AddWithValue("@FechaFin", $FechaFin) | Out-Null
    ...
    Write-Log "Registros totales obtenidos del SP para el backfill: $totalRegistros"

    # ─── 5. Verificar si hay datos ───────────────────────────────────────────
    if ($totalRegistros -eq 0) {
        Write-Log "No hay viajes para el rango $FechaInicio a $FechaFin. Finalizando sin envío."
    ...
    # ─── 7. INSERT: enviar el lote completo  ─────────────────────────────────
    # Si un Id_Viaje ya existe en Supabase, se salta y continua con el siguiente
    # Si la lista de registros es excesivamente larga (>50,000) podría requerir
    # particionamiento, pero para rangos backfill normales está bien.
    Write-Log "Enviando $totalRegistros registros a Supabase (bulk INSERT, ignorando duplicados si existen)..."
    ...
    $debugFile = Join-Path $LogDir ("debug_payload_backfill_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".json")
    ...
    try {
        # Timeout extendido por ser backfill masivo
        $insertResp = Invoke-WebRequest -Uri $insertUrl -Method POST `
                          -Headers $insertHeaders -Body $utf8Body -UseBasicParsing -ErrorAction Stop -TimeoutSec 300
    ...
    Write-Log "========================================================"
    Write-Log "Sincronización BACKFILL completada exitosamente. $totalRegistros registros enviados."
    Write-Log "========================================================"
    ...
    Write-Log "ERROR durante el BACKFILL: $mensajeError" "ERROR"
